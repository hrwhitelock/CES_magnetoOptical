...
HMF = @(HCEFm, JJab, hv, mv) HCEFm - (z/2)*(mv*JJab*mv')*ID...
    - (muB*gJ*hv(1) - (z/2)*(JJab(:,1)'+JJab(1,:))*mv')*Jx... % - (muB*gJ*hv(2) - (z/2)*(JJab(:,2)'+JJab(2,:))*mv')*Jy...
...
% at a temperature t
    function out = M(HCEFm, JJab, hv, t)
        ...
        %f2 = @(mx,my,mz) trace( Jy*expm(-B*HMF(HCEFm, JJab, hv, [mx,my,mz])) )/trace( expm(-B*HMF(HCEFm, JJab, hv, [mx,my,mz])) ) - my;
