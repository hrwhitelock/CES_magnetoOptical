...
ampC, arrC = zeemanSplitLinesC(fieldArr, B20, B40, B43, B60, B63, B66, 0)
arrC = np.array(arrC)
arrC = arrC*meVToCm
arrC = arrC.T
...
with h5py.File('zeeman_split_lines.h5', 'w') as hdf:
...
[[Fit Statistics]]
    ...
    # data points      = 116493
    # variables        = 1
    chi-square         = 1292.46226
    reduced chi-square = 0.01109486
    Akaike info crit   = -524365.851
    Bayesian info crit = -524356.186
    R-squared          = -0.08783922
[[Variables]]
    B20:   -0.03559 (fixed)
    B40:   -0.0003849 (fixed)
    B43:   -0.01393 (fixed)
    ...
    B63:   -4.695e-06 (fixed)
    B66:    3.3815e-05 (fixed)
    Jperp: -5.7549e-04 +/- 2.6149e-06 (0.45%) (init = -0.0002)
...
ampB_neg, arrB_neg = zeemanSplitLinesAB(fieldArr, B20, B40, B43, B60, B63, B66, Jz)
arrB_neg = np.array(arrB_neg) * meVToCm
arrB_neg = arrB_neg.T
ampB_neg = np.array(ampB_neg).T

ampB_zero, arrB_zero = zeemanSplitLinesC(fieldArr, B20, B40, B43, B60, B63, B66, 0)
